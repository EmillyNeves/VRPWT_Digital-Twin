...
% ==================================================================
Sob o critério de parada por iterações sem melhoria ($K=\Kgrasp$ para os três métodos; Seção~\ref{sec:calibracao}), com os parâmetros calibrados por \textit{irace} (valores em \texttt{experiments/config/tuned.json}), executamos as 56 instâncias (30 execuções para os métodos estocásticos). A Tabela~\ref{tab:overall} resume o desempenho global e a Tabela~\ref{tab:familia} o detalha por família. O tempo e o número de iterações são reportados para tornar transparente o esforço de cada método (uma iteração do GRASP é muito mais cara que uma da Tabu).
...
% ATENCAO: as colunas "Nosso (GRASP)" desta tabela e as medias citadas no
% paragrafo acima vem do ESTUDO ANTIGO (as colunas das referencias sao estaveis).
% Regenerar via analysis/lexicographic_table.py apos o estudo novo.
\begin{table}[H]
\centering
...
% ==================================================================
% ATENCAO: os numeros deste paragrafo e da Tabela tab:twin vem da
% simulacao ANTERIOR a camada LoRaWAN (canal implicito perfeito). Serao
% regenerados por 3_digital_twin.sh com o canal padrao (pdr=0,98), e os
% valores mudam. Nao editar a mao.
% ==================================================================
...
% ATENCAO: numeros deste paragrafo vem do ESTUDO ANTIGO; reescrever a partir
% dos artefatos gerados pelo estudo novo (2_run_study). Nao editar a mao.
