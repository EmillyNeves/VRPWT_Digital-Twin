...
\else
  \setmainfont{Arial}\setsansfont{Arial}\renewcommand{\familydefault}{\sfdefault}
\fi
...
% K fixado pelo joelho da convergência (preenchido a partir de config/fixed_K.json).
\newcommand{\Kgrasp}{250}
\newcommand{\Krgrasp}{250}
\newcommand{\Ktabu}{1400}

% Resultado do integral primal (preenchido a partir de results/primal_integral.csv).
\newcommand{\PIRESULT}{GRASP ($\mathit{PI}=3{,}59$), GRASP reativo ($3{,}67$) e Busca Tabu ($3{,}77$) ficaram \emph{próximos entre si}, todos com cerca de \emph{metade} do integral primal do VND ($7{,}66$) --- confirmando, também na dimensão anytime, o ordenamento da comparação primária.}
...
\end{enumerate}
A consequência prática (Seção~\ref{sec:viabilidade}) é que, nas dezenas de milhares de execuções realizadas, \textbf{nenhuma} solução inviável foi produzida por qualquer algoritmo. O critério de aceitação de \emph{todos} os operadores de melhoria descarta movimentos inviáveis \emph{antes} de aplicá-los (Seção~\ref{sec:aceitacao}); a viabilidade é, portanto, um invariante preservado a cada passo.
...
O fluxo de uma execução é o seguinte: \emph{(1)} o arquivo da instância é lido e \emph{(2)} a matriz de distâncias é construída; \emph{(3)} gera-se uma solução inicial --- a I1 (Algoritmo~\ref{alg:i1}) para VND e Busca Tabu, ou a construção gulosa-aleatorizada para o GRASP; \emph{(4)} a busca local/meta-heurística refina a solução, consultando o avaliador e as vizinhanças e aceitando movimentos conforme as ``regras do jogo'' (Seção~\ref{sec:aceitacao}), até o critério de parada (Seção~\ref{sec:parada}); \emph{(5)} a solução final é \textbf{revalidada} do zero pelo verificador independente; e \emph{(6)} os artefatos são gravados: o arquivo \texttt{.sol} (rotas), uma linha de CSV (distância, veículos, iterações, tempo, viabilidade), o traço de convergência e, opcionalmente, instantâneos por movimento.
...
\textbf{Reprodutibilidade e instrumentação.} O gerador pseudoaleatório é semeado misturando, de forma determinística, uma semente base, um \emph{hash} do nome da instância e o índice da execução, de modo que cada execução é reproduzível a partir da semente registrada. A cada execução o solver grava: o arquivo \texttt{.sol} (rotas e custo), uma linha de CSV (algoritmo, instância, semente, distância, veículos, tempo, viabilidade e iterações), o traço de convergência (tempo, melhor custo) e, opcionalmente, instantâneos por movimento para as figuras de evolução. O \texttt{Validator}, independente, reavalia a solução do zero --- o \emph{portão} de corretude.

\textbf{Pipeline experimental.} O \texttt{runner.py} invoca o binário do solver via \texttt{subprocess} para cada $(\text{algoritmo},\text{instância},\text{semente})$, em paralelo (\texttt{ThreadPoolExecutor}), consolidando o CSV mestre e aplicando os parâmetros calibrados (\texttt{tuned.json}). A calibração usa o \textit{irace} por meio de um \texttt{target-runner} que imprime o gap; os módulos de análise computam as agregações, os testes de Friedman/Nemenyi (\texttt{scipy} $+$ \texttt{scikit-posthocs}) e as figuras.
...
Adotamos um \emph{kit} padronizado de cinco vizinhanças, compartilhado por VND, GRASP e Busca Tabu:
...
\item \textbf{Or-opt} (intra e entre rotas): move uma cadeia de 2--3 clientes, eventualmente invertida;
\item \textbf{Cross-exchange} (entre rotas): troca subtrechos de até 3 clientes entre duas rotas.
\end{itemize}
Esses operadores são clássicos para o VRPTW \citep{potvin1995exchange,toth2002vehicle}. Este \emph{kit} é o \textbf{subconjunto selecionado} a partir do conjunto mais amplo de operadores avaliados no relatório parcial: o 2-opt é mantido na forma clássica \emph{intra-rota}, enquanto a reorganização \emph{entre} rotas (papel que seria do ``2-opt inter'') é exercida pelo Cross-exchange e pelo Or-opt entre rotas, que trocam/realocam subtrechos entre veículos. A escolha do \emph{kit completo} é justificada empiricamente por uma ablação (Seção~\ref{sec:ablacao}): a remoção de qualquer operador piora a qualidade média. Cada operador avalia a viabilidade do movimento por uma simulação completa da(s) rota(s) afetada(s), de modo que nenhum movimento inviável é jamais aplicado (Seção~\ref{sec:aceitacao}). A Figura~\ref{fig:operadores} ilustra o efeito de cada um.
...
\textbf{Leitura passo a passo.} O índice $\ell$ (linha~\ref{vnd:init}) aponta a vizinhança ativa. A linha~\ref{vnd:find} varre \emph{toda} a vizinhança $N_\ell$ e retorna o melhor movimento melhorante (estratégia de \emph{melhor melhora}, que escolhe o movimento de maior redução, não o primeiro). Se há ganho, a linha~\ref{vnd:apply} o aplica e \textbf{reinicia de $\ell=1$}: voltar à vizinhança mais barata após cada melhoria é o que caracteriza o VND e o que torna a descida sistemática. Sem ganho, a linha~\ref{vnd:next} passa à vizinhança seguinte; o laço encerra quando nenhuma das $p$ vizinhanças melhora $S$. Em termos da \emph{formação da solução}, é aqui que a rota rígida da I1 começa a ser reorganizada: o Relocate corrige clientes mal alocados, o Or-opt e o Cross-exchange transferem trechos \emph{entre} veículos, e o 2-opt desfaz cruzamentos \emph{dentro} de uma rota --- reduzindo a distância de $35{,}97\%$ (I1) para $11{,}07\%$ de gap médio, ainda preso, porém, ao primeiro ótimo local encontrado.
...
O GRASP \citep{feo1995greedy,resende2003greedy} repete um ciclo \emph{construção gulosa-aleatorizada $\rightarrow$ busca local} e guarda a melhor solução. A construção reusa a I1, mas substitui a escolha gulosa do melhor cliente por uma seleção aleatória dentro de uma \textbf{Lista Restrita de Candidatos} (RCL) baseada em valor: dado o conjunto de candidatos com valores $c_2$, sejam $c^{\max}$ e $c^{\min}$ o maior e o menor; a RCL contém todo candidato com
\begin{equation}
...
\STATE $S^\star \gets \varnothing$;\quad $k \gets 0$ \label{g:init}
\WHILE{critério de parada não satisfeito}
  ...
  \IF{$S^\star=\varnothing$ \textbf{ ou } distância$(S) <$ distância$(S^\star)$}
...
\textbf{Leitura passo a passo.} Cada iteração é um ciclo \emph{construção $\rightarrow$ busca local}: a linha~\ref{g:build} gera uma solução nova com aleatorização controlada por $\alpha$ (a RCL da Figura~\ref{fig:rcl}), e a linha~\ref{g:ls} a conduz a um ótimo local pelo VND (Algoritmo~\ref{alg:vnd}). A linha~\ref{g:improve} retém a melhor solução já vista e zera o contador de estagnação; a linha~\ref{g:noimprove} o incrementa quando não há ganho; e a linha~\ref{g:stop} encerra após $K$ iterações sem melhora. O que o GRASP acrescenta à formação da solução é \emph{diversificação estruturada}: ao reconstruir a rota do zero a cada iteração, ele rompe a dependência da semente única da I1 --- cada RCL produz uma topologia de rotas diferente ---, e a melhor entre centenas de descidas independentes leva o gap médio à faixa de $3{,}4\%$.

\subsection{GRASP Reativo}\label{sec:rgrasp}
No GRASP Reativo \citep{prais2000reactive}, em vez de um $\alpha$ fixo, mantém-se um conjunto discreto $\{\alpha_1,\dots,\alpha_g\}$ (grade $0{,}05,0{,}10,\dots,0{,}50$) com probabilidades $p_i$ de seleção. A cada bloco de \textit{block} iterações, as probabilidades são atualizadas de modo que valores de $\alpha$ que produziram soluções melhores fiquem mais prováveis:
\begin{equation}
...
\ENSURE melhor solução $S^\star$
\STATE $p_i \gets 1/g$,\ $\Sigma_i \gets 0$,\ $n_i \gets 0$ \textbf{para todo} $i$;\quad $S^\star \gets \varnothing$;\quad $k \gets 0$;\quad $\mathit{it} \gets 0$ \label{rg:init}
\WHILE{critério de parada não satisfeito}
  \IF{$\mathit{it} < g$}
  ...
  \IF{$S^\star=\varnothing$ \textbf{ ou } $c <$ distância$(S^\star)$}
...
\WHILE{critério de parada não satisfeito}
  ...
  \IF{não existe movimento admissível}
    \STATE \textbf{interrompa} \label{t:deadend}
  \ENDIF
...
\textbf{Leitura passo a passo.} A cada iteração, a linha~\ref{t:find} varre todo o kit de vizinhanças e escolhe o melhor movimento \emph{admissível} --- não-tabu ou, se tabu, que satisfaça a \emph{aspiração} (produzir um novo melhor global). Diferentemente do VND, a Tabu \textbf{não} exige que o movimento melhore: a linha~\ref{t:apply} pode aplicar uma piora controlada, e é assim que a busca escapa de ótimos locais. O movimento torna tabu, por \textit{tenure} iterações, o(s) cliente(s) que ele toca (memória de curto prazo), o que impede a busca de reverter movimentos recentes e ciclar. A linha~\ref{t:improve} guarda o melhor já visto. Na formação da solução, a Tabu percorre uma \emph{única} trajetória contínua, refinando a estrutura herdada da I1 sem reinícios --- o que explica seu desempenho competitivo ($4{,}51\%$ de gap), porém ligeiramente atrás do GRASP, que explora múltiplas estruturas em paralelo.

\subsection{Critérios de aceitação --- ``as regras do jogo''}\label{sec:aceitacao}
...
A calibração é feita com o pacote \textit{irace} \citep{lopez2016irace}, padrão de fato para configuração automática de algoritmos. Para evitar superajuste, as 56 instâncias são divididas em \textbf{treino} (28) e \textbf{teste} (28), estratificadas por família (C/R/RC) e tipo (1/2); o \textit{irace} usa apenas o treino, e os resultados são reportados no teste e no conjunto completo. Adotamos um desenho \textbf{desacoplado}: o critério de parada $K$ é \emph{fixado} por análise de convergência (a seguir), e o \textit{irace} calibra \emph{apenas} os hiperparâmetros de qualidade de cada método --- GRASP: $\alpha$; Reativo: $\delta$ e \textit{block}; Tabu: \textit{tenure}. Os demais elementos (parâmetros da I1, conjunto de vizinhanças, número de execuções) são \emph{fixados} por justiça. A função-objetivo do \textit{irace} é o \emph{gap} de distância ao melhor valor conhecido, que normaliza escalas entre instâncias. A justificativa de cada faixa de busca e da separação ``calibrar vs.\ fixar'' está documentada no arquivo \texttt{TUNING\_RATIONALE}.

\textbf{Fixação de $K$ por convergência.} O critério de parada $K$ é um \emph{orçamento}: mais iterações sem melhora nunca pioram o gap. Calibrá-lo pelo gap, junto aos parâmetros de qualidade, o levaria ao teto do intervalo de busca --- conflacionando \emph{orçamento} e \emph{qualidade}. Adotamos, por isso, a abordagem metodologicamente correta de \textbf{desacoplar}: fixamos $K$ pelo \emph{joelho} da curva gap$\times K$ nas instâncias de treino (Figura~\ref{fig:knee}), escolhendo o menor $K$ cujo gap médio dista menos de $0{,}1$ ponto percentual do gap obtido com o maior $K$ testado; só então o \textit{irace} calibra os parâmetros de qualidade \emph{a esse $K$ fixo}. Os valores resultantes são $K_{\mathrm{GRASP}}=\Kgrasp$, $K_{\mathrm{Reativo}}=\Krgrasp$ e $K_{\mathrm{Tabu}}=\Ktabu$. Empiricamente, a maior parte do ganho ocorre cedo (a melhor solução costuma ser encontrada bem antes do joelho); reportamos o tempo, o número de iterações e as curvas tempo-alvo (TTT), tornando o esforço transparente. Esse desenho separa claramente o \emph{orçamento} (fixado por convergência) da \emph{qualidade} (calibrada pelo irace).

\begin{figure}[htbp]
\centering
\includegraphics[width=0.72\textwidth]{../../results/figures/knee_K}
\caption{Seleção do critério de parada $K$ pelo \emph{joelho} da curva gap$\times K$ (instâncias de treino): a partir do joelho, mais paciência rende retorno desprezível. Fixa-se o $K$ do joelho de cada método.}\label{fig:knee}
\end{figure}

\subsection{Comparação estatística}\label{sec:estatistica}
Cada algoritmo estocástico é executado 30 vezes por instância, com sementes independentes registradas; os determinísticos (I1, VND, Busca Tabu) executam uma vez, por não possuírem fonte de aleatoriedade. Para comparar os métodos sobre as 56 instâncias usamos o teste não-paramétrico de \textbf{Friedman} sobre os \emph{ranks} por instância (variável-resposta: gap médio), seguido do pós-teste de \textbf{Nemenyi} (todas as comparações par a par), conforme \citet{demsar2006statistical}. A estatística de Friedman, \emph{com correção para empates} (frequentes, pois na família C vários métodos atingem o ótimo), é
\begin{equation}
\chi^2_F=\frac{1}{C}\cdot\frac{12N}{k(k+1)}\left[\sum_{j=1}^{k}R_j^2-\frac{k(k+1)^2}{4}\right],
\qquad C=1-\frac{\sum_g (t_g^3-t_g)}{N\,(k^3-k)},
\end{equation}
com $N$ instâncias, $k$ algoritmos, $R_j$ o \emph{rank} médio do algoritmo $j$ (rank $1$ = melhor) e $t_g$ o tamanho de cada grupo de empate. Sem a correção ($C=1$) obter-se-ia $\chi^2_F\approx167{,}1$; com ela, $\chi^2_F=184{,}24$ (o valor reportado na Seção~\ref{sec:comparacao}). Esse é o cálculo realizado pela implementação (\texttt{scipy.stats.friedmanchisquare}). No pós-teste de Nemenyi, dois algoritmos diferem significativamente se seus ranks médios distam mais que a \emph{diferença crítica}
\begin{equation}
...
Os experimentos foram conduzidos em uma única máquina --- Intel\textsuperscript{\textregistered} Core\textsuperscript{\texttrademark} Ultra~9 285K (24 \textit{threads}), Ubuntu~24.04 LTS (kernel 6.17), com o solver compilado por \texttt{g++}~13.3 e \texttt{-std=c++20 -O2}. Cada execução é reprodutível a partir da semente registrada: a semente combina, de forma determinística, um valor base, o nome da instância e o índice da execução. A convenção de distância e a corretude do avaliador são verificadas contra soluções de referência (Seção~\ref{sec:verificacao}).
...
Em mais de sete mil execuções (construção, VND, GRASP, Reativo e Tabu, em todas as instâncias e sementes), o verificador independente não acusou \textbf{nenhuma} solução inviável, e nenhum \emph{gap} negativo (distância inferior à referência) ocorreu. Isso confirma, empiricamente, o invariante da Seção~\ref{sec:invariantes}: os algoritmos de melhoria formam e modificam rotas \emph{sem jamais violar} cobertura, capacidade ou janelas de tempo.

\subsection{Ablação do conjunto de vizinhanças}\label{sec:ablacao}
A Tabela~\ref{tab:ablacao} mostra o efeito de remover cada operador do kit completo (VND, 56 instâncias). Remover \emph{qualquer} operador piora o gap médio, o que justifica a adoção do kit completo. A hierarquia de importância é clara: Relocate é o operador dominante, seguido de Or-opt e Cross-exchange.

\begin{table}[H]
\centering
\caption{Ablação das vizinhanças (VND; gap médio de distância nas 56 instâncias).}\label{tab:ablacao}
\begin{tabular}{lcc}
\toprule
\textbf{Configuração} & \textbf{Gap médio (\%)} & \textbf{$\Delta$ vs.\ kit completo} \\
\midrule
Kit completo (5 operadores)        & 11{,}07 & --- \\
\quad sem Relocate                 & 15{,}64 & $+4{,}56$ \\
\quad sem Or-opt                   & 12{,}60 & $+1{,}53$ \\
\quad sem Cross-exchange           & 11{,}91 & $+0{,}83$ \\
\quad sem 2-opt                    & 11{,}53 & $+0{,}46$ \\
\quad sem Swap                     & 11{,}18 & $+0{,}11$ \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Comparação de desempenho}\label{sec:comparacao}
Sob o critério de parada por iterações sem melhoria, com os parâmetros recalibrados por \textit{irace} (GRASP $\alpha=0{,}1517$, $K=\Kgrasp$; Reativo $\delta=1{,}3992$, \textit{block}$=199$, $K=\Krgrasp$; Tabu \textit{tenure}$=39$, $K=\Ktabu$), executamos as 56 instâncias (30 execuções para os métodos estocásticos). A Tabela~\ref{tab:overall} resume o desempenho global e a Tabela~\ref{tab:familia} o detalha por família. O tempo e o número de iterações são reportados para tornar transparente o esforço de cada método (uma iteração do GRASP é muito mais cara que uma da Tabu).

\begin{table}[H]
\centering
\caption{Desempenho global nas 56 instâncias (gap de distância ao melhor conhecido).}\label{tab:overall}
...
GRASP           & 3{,}40 & \textbf{1{,}92} & 8{,}21 & 390 & 15{,}3 \\
GRASP reativo   & \textbf{3{,}38} & 2{,}00 & 8{,}28 & 390 & 18{,}5 \\
...
\begin{table}[H]
\centering
\caption{Gap médio de distância por família (\%).}\label{tab:familia}
\begin{tabular}{lccc}
\toprule
...
\paragraph{Significância estatística.} O teste de Friedman rejeita fortemente a hipótese de igualdade entre os métodos ($\chi^2=184{,}24$, $p\approx 9{,}2\times 10^{-39}$, 56 instâncias). Os \emph{ranks} médios (menor é melhor) são: GRASP $1{,}82$, GRASP reativo $1{,}91$, Busca Tabu $2{,}48$, VND $3{,}79$ e I1 $5{,}00$. O pós-teste de Nemenyi (Figura~\ref{fig:cd}) indica que \textbf{GRASP, GRASP reativo e Busca Tabu não diferem significativamente entre si} --- o menor $p$ entre os três pares é $0{,}18$ (GRASP\,$\times$\,Tabu), e suas diferenças de \emph{rank} ficam abaixo da diferença crítica $CD=0{,}815$ --- enquanto \textbf{todos são significativamente superiores} a VND e I1 ($p<0{,}001$). O mesmo padrão se confirma no conjunto de teste, livre de superajuste da calibração (28 instâncias; $\chi^2=96{,}20$, $p\approx 6{,}3\times 10^{-20}$; as três meta-heurísticas permanecem indistinguíveis, $p\ge 0{,}29$).

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../../results/figures/cd_diagram_full}
\caption{Diagrama de diferença crítica (Friedman/Nemenyi, 56 instâncias). Métodos unidos por uma barra horizontal não diferem significativamente ($p>0{,}05$).}\label{fig:cd}
\end{figure}

\paragraph{Convergência e tempo-alvo (TTT).} A Figura~\ref{fig:conv} ilustra a queda do custo ao longo do tempo na instância R101. A análise tempo-alvo na mesma instância (Figura~\ref{fig:ttt}), com alvo a $1\%$ do melhor conhecido, mostra que \textbf{$100\%$ das $100$ execuções independentes atingiram o alvo}, com a distribuição empírica do tempo necessário --- evidência da robustez do GRASP.
...
\paragraph{Resultados (prova de conceito em Vitória/ES).} Sobre uma simulação de 16 ciclos com a mesma realização de enchimento, o regime \textbf{dinâmico} alcançou distância comparável ao \textbf{estático} ($3550{,}6$ vs.\ $3566{,}3$) com cerca de \emph{metade das coletas} ($119$ vs.\ $228$) e \emph{menos transbordos} ($16$ vs.\ $25$) --- evidência de que coletar sob demanda, guiado pelos sensores, é mais eficiente que a coleta de frequência fixa (Tabela~\ref{tab:twin}). Aplicando os cinco algoritmos à mesma simulação, todos os de melhoria (VND, GRASP, Reativo, Tabu) convergiram à mesma solução ($3550{,}6$), superando a construção pura I1 ($3632{,}2$): nas instâncias \emph{pequenas} de cada ciclo a diferença entre as meta-heurísticas desaparece --- as distinções observadas no \textit{benchmark} de 100 clientes manifestam-se em escala maior.

\begin{table}[H]
\centering
\caption{Digital Twin (Vitória/ES, 16 ciclos): coleta dinâmica vs.\ estática (frequência fixa).}\label{tab:twin}
\begin{tabular}{lcccc}
\toprule
\textbf{Regime} & \textbf{Distância} & \textbf{Coletas} & \textbf{Transbordos} & \textbf{Ciclos com rota} \\
\midrule
Dinâmico (sob demanda) & 3550{,}6 & 119 & \textbf{16} & 14 \\
...
Quantitativamente, as três meta-heurísticas (GRASP, GRASP reativo e Busca Tabu) mostraram-se \textbf{estatisticamente equivalentes entre si} e \textbf{significativamente superiores} ao VND e à construção I1, reduzindo o gap médio de distância de $35{,}97\%$ (I1) e $11{,}07\%$ (VND) para a faixa de $3{,}4$--$4{,}5\%$ ao melhor conhecido; o GRASP obteve o melhor \emph{rank} médio. A verificação reproduziu os $56$ custos de referência e \emph{nenhuma} das milhares de soluções produzidas foi inviável, confirmando que os algoritmos de melhoria respeitam todas as restrições a cada passo. Transportados ao Digital Twin de coleta em Vitória/ES, os mesmos algoritmos calibrados mostraram que a coleta dinâmica guiada por sensores reduz transbordos frente à coleta de frequência fixa, como prova de conceito.
